...
public class PaymentController {
  ...
  public CreatePaymentCustomerResponse createCustomer(
      @PathVariable String userId, @RequestBody CreatePaymentCustomerRequestBody customer)
      throws StripeException {
    Customer created =
        paymentService.createCustomer(Objects.requireNonNull(customer.getData()), userId);
  ...
  public PaymentMethodResponse getPaymentMethods(@PathVariable String userId)
    ...
    List<PaymentMethod> data =
        paymentService.getPaymentMethods(userId).stream().map(paymentMapper::toRest).toList();
  ...
  public PaymentMethodResponse attachPaymentMethod(
      @PathVariable String paymentMethodId, @PathVariable String userId) throws StripeException {
    List<PaymentMethod> data =
        paymentService.attach(paymentMethodId, userId).stream().map(paymentMapper::toRest).toList();

    return new PaymentMethodResponse().data(data);
  }

  @PutMapping("/users/{userId}/payment-methods/{paymentMethodId}/detach")
  public PaymentMethodResponse detachPaymentMethod(
      @PathVariable String paymentMethodId, @PathVariable String userId) throws StripeException {
    List<PaymentMethod> data =
        paymentService.detach(paymentMethodId, userId).stream().map(paymentMapper::toRest).toList();

    return new PaymentMethodResponse().data(data);
  }

  @PutMapping("/users/{userId}/payment-methods/{paymentMethodId}/set-default")
  public CreatePaymentCustomerResponse setDefaultPaymentMethod(
      @PathVariable String paymentMethodId, @PathVariable String userId) throws StripeException {
    PaymentCustomer data =
        paymentMapper.restCustomer(paymentService.setDefault(paymentMethodId, userId));
    return new CreatePaymentCustomerResponse().data(data);
  }
}
