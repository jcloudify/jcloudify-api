...
public class LambdaFunctionLogService {
    ...
    public void crupdateLogGroups(String functionName, String bucketKey) {
        List<LogGroup> logGroups = mapper.toRest(cloudwatchComponent.getLambdaFunctionLogGroupsByName(functionName));
        try {
            File logGroupsFile;
    ...
    public Page<LogGroup> getLogGroups(String userId, String applicationId, String environmentId, String functionName,
                                       PageFromOne page, BoundedPageSize pageSize) {
        String logGroupBucketKey = getLogGroupsBucketKey(userId, applicationId, environmentId, functionName);
        try {
            List<LogGroup> logGroups = fromStackDataFileToList(bucketComponent, om, logGroupBucketKey, LogGroup.class);
    ...
    public static String getLogGroupsBucketKey(String userId, String applicationId, String environmentId, String functionName) {
        return String.format("users/%s/apps/%s/envs/%s/function/%s/logGroups/%s", userId, applicationId, environmentId, functionName, "log-group.json");
    }
}
